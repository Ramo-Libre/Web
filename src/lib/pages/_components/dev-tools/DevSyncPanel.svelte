		...
		switch (feature) {
			...
			case 'schedule': {
				...
				for (let i = 0; i < count; i++) {
					...
					if (isRecurring) {
						...
						event.recurrenceEnd = faker.date
							.soon({ days: 90, refDate: start })
							.toISOString()
							.slice(0, 10);
					} else {
			...
			case 'preferences': {
				return {
					...
					schedule: {
						showCalendarEvents: faker.datatype.boolean(),
						orientation: faker.helpers.arrayElement(['normal', 'rotated'])
					},
			...
			case 'semesters': {
				...
				for (let i = 0; i < faker.number.int({ min: 1, max: 3 }); i++) {
					sems.push([
						generateUUID(),
						{ name: faker.date.anytime().toLocaleString('es', { year: 'numeric', month: 'long' }) }
					]);
				}
	...
	<div class="p-3 space-y-2.5">
		<div class="grid grid-cols-2 gap-2">
			<div class="flex flex-col gap-0.5">
				<label
					for="sync-feature"
					class="text-[9px] font-bold text-content/40 uppercase tracking-wider">Entidad</label
				>
				<select
					id="sync-feature"
			...
			<div class="flex flex-col gap-0.5">
				<label
					for="sync-action"
					class="text-[9px] font-bold text-content/40 uppercase tracking-wider">Método</label
				>
				<select
					id="sync-action"
		...
		{#if lastEvent}
			<div class="bg-base-300/60 border border-base-400 rounded-lg p-2 space-y-0.5">
				<div class="text-[10px] font-bold text-content/40 uppercase tracking-wider mb-1">
					Último evento
				</div>
				...
				<pre
					class="text-[10px] text-content/40 leading-tight mt-1 max-h-[120px] overflow-auto custom-scroll">{JSON.stringify(
						lastEvent.payload,
						null,
						1
					)}</pre>
