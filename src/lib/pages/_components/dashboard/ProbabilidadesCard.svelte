<script lang="ts">
	...
	function escStatus(e: { lastResult?: { feasible: boolean; probability: number } | null }): {
		...
		if (e.lastResult.feasible && e.lastResult.probability >= 0.9999)
			return { label: 'Garantizado', cls: 'text-success-100', Icon: Award };
		if (e.lastResult.feasible)
			return { label: 'Factible', cls: 'text-primary-100', Icon: CircleCheck };
	...
	const escTotal = $derived(ramoEscenarios.reduce((sum, r) => sum + r.escenarios.length, 0));
</script>
	...
	{#if escTotal > 0}
		<div class="space-y-1">
			{#each ramoProbBars as ramo (ramo.id)}
				<div
					class="flex items-center gap-2.5 w-full rounded-lg px-3 py-2 hover:bg-base-200 transition-colors select-none!"
				>
					...
					<div class="flex-1 h-3 bg-base-300 rounded-full overflow-hidden">
						<div
							...
							style="width: {Math.min(100, ramo.prob * 100)}%"
						></div>
					</div>
					<span
						class="text-xs font-bold tabular-nums {ramo.label === 'Sin datos'
							? 'text-content/30'
							: 'text-content/60'} w-10 text-right">{ramo.pctText}</span
					>
				</div>
			{/each}
		</div>
