	...
	{:else}
		<div class="space-y-4">
			...
			<div class="flex flex-wrap items-center gap-3">
				<span class="text-xs font-semibold text-content/50 uppercase tracking-wider">Seleccionar</span>
				<button
					onclick={() => { for (const e of events) e.selected = true; }}
					class="px-2.5 py-1 rounded-lg border text-xs font-medium transition-all cursor-pointer {events.every((e) => e.selected)
				...
				<button
					onclick={() => { for (const e of events) e.selected = false; }}
					class="px-2.5 py-1 rounded-lg border text-xs font-medium transition-all cursor-pointer {!events.some((e) => e.selected)
						? 'bg-primary-100 text-base-100 border-primary-100'
						: 'bg-base-50 text-content/40 border-base-400 hover:border-primary-100 hover:text-content'}"
				>
					Ninguno
