<script lang="ts">
	...
	import { Plus, ChevronDown, Calendar, List } from '@lucide/svelte';
...
<div class="bg-base-100 border border-base-400 rounded-xl p-4">
		...
		<div class="grid grid-cols-[1fr_auto] gap-1.5">
			...
			<div class="flex gap-1.5">
				...
				<button
					...
					class="flex items-center justify-center px-3 py-1.5 rounded-lg border text-sm font-semibold transition-all cursor-pointer {showLista
						? 'bg-primary-100 text-base-100 border-primary-100'
						: 'bg-base-100 text-content/40 border-base-400 hover:border-primary-100 hover:text-content'}"
	...
	<div class="hidden sm:flex items-center gap-3">
		<div class="flex items-center gap-3 flex-wrap">
			...
			<button
				onclick={onToggleLista}
				class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-semibold transition-all cursor-pointer shrink-0 {showLista
					? 'bg-primary-100 text-base-100 border-primary-100'
					: 'bg-base-100 text-content/60 border-base-400 hover:border-primary-100 hover:text-content'}"
				title="Vista de lista"
			>
