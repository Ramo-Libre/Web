<script lang="ts">
	...
	import { Plus, ChevronDown, Presentation, CircleAlert, Book, FlaskConical, Users, Wrench, Ellipsis, Clock } from '@lucide/svelte';
...
<div class="bg-base-100 border border-base-400 rounded-xl p-4">
	...
	<div class="sm:hidden grid grid-cols-[1fr_auto] gap-1.5">
		...
		<div class="flex flex-col gap-1.5 justify-between">
			...
			<button
				onclick={onAddEvent}
				class="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-100 text-base-100 text-sm font-semibold hover:opacity-90 transition-opacity cursor-pointer"
			>
		<Plus class="w-4 h-4" />
			</button>
		</div>
