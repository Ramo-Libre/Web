<script lang="ts">
	...
	const maxHour = $derived(
		timed.length > 0
			? Math.max(
					...timed.map((e) => parseInt((e.endTime || e.startTime)!.split(':')[0]))
				)
			: 23
	);
...
<div class="bg-base-100 border border-base-400 rounded-xl">
	...
	{:else}
		<div class="flex items-center justify-between p-5 pb-3 border-b border-base-300">
			...
			<span class="text-xs font-mono text-content/40">{events.length} evento{events.length !== 1 ? 's' : ''}</span>
		</div>
		...
		{:else}
			<div class="p-5 pt-4">
				{#if allDay.length > 0}
					<div class="mb-5">
						<span class="text-[11px] font-semibold text-content/40 tracking-wider uppercase">Todo el día</span>
							...
							{#each allDay as event (event.id)}
								...
								>
									<span
										...
										style="background: {ramoColor(event.ramoId)}20; color: {ramoColor(event.ramoId)}"
									>
										<Icon class="w-4 h-4" />
									</span>
						...
						{#each hourRange as hour (hour)}
							...
							<div class="flex gap-4 py-0.5">
								<div class="w-10 shrink-0 pt-1.5 text-right">
									<span class="text-[11px] font-mono text-content/30">{String(hour).padStart(2, '0')}:00</span>
								</div>
										...
										>
											...
											<div class="flex-1 min-w-0">
												<div class="flex items-center gap-2">
													<span class="text-[11px] font-mono text-primary-100 font-medium shrink-0">{timeLabel}</span>
													<span class="font-semibold text-sm text-content truncate">
														{event.title || categoryLabels[event.category] || 'Evento'}
													</span>
													{#if event.ramoId}
														<span class="w-2 h-2 rounded-full shrink-0" style="background: {ramoColor(event.ramoId)}"></span>
													{/if}
												</div>
												{#if event.description}
													<p class="text-xs text-content/50 mt-0.5 line-clamp-2">{event.description}</p>
												{/if}
												{#if event.ramoId}
													<span class="text-[11px] text-content/30">{ramoName(event.ramoId)}</span>
												{/if}
