<script lang="ts">
    ...
    // Detectar estados
    const currentClass = $derived(todayClasses.find(c => currentMin >= c.startMin && currentMin < c.endMin));
    ...
    const isDayFinished = $derived(todayClasses.length > 0 && !currentClass && !nextClass && currentMin >= todayClasses[todayClasses.length - 1].endMin);
    ...
    const primaryColor = $derived(currentClass ? currentClass.color : (nextClass ? nextClass.color : '#cbd5e1'));
</script>

<div class="bg-white rounded-2xl border border-slate-200 shadow-inner flex flex-col overflow-hidden relative isolate">
    ...
    <div class="p-6 flex-1 flex flex-col items-center justify-center">
        ...
        <div class="relative w-[280px] h-[280px] sm:w-[320px] sm:h-[320px] mb-8 flex-shrink-0">
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                ...
                {#if currentClass}
                    <span class="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-2 flex items-center gap-1.5">
                        <div class="w-2 h-2 rounded-full animate-pulse" style="background-color: {primaryColor}"></div>
                        Termina en
                    </span>
                    <h2 class="text-4xl sm:text-5xl font-black text-slate-800 tabular-nums tracking-tight mb-2">
                        {countdownStr}
                    </h2>
                ...
                {:else if nextClass}
                    <span class="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
                        Próxima en
                    </span>
                    <h2 class="text-4xl sm:text-5xl font-black text-slate-800 tabular-nums tracking-tight mb-2 opacity-80">
                        {countdownStr}
                    </h2>
                    <p class="font-bold text-sm sm:text-base text-slate-600 truncate w-full">
                        {nextClass.ramoNombre}
                    </p>
                    <div class="flex items-center justify-center gap-3 text-xs text-slate-500 font-medium mt-1">
                        <span class="flex items-center gap-1"><Clock class="w-3 h-3"/> {nextClass.start}</span>
                        {#if nextClass.location}
                            <span class="flex items-center gap-1"><MapPin class="w-3 h-3"/> {nextClass.location}</span>
                        {/if}
                    </div>
        ...
        <div class="w-full max-w-sm mt-4">
            {#if currentClass}
                <div class="text-center mb-4">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border border-slate-200 text-xs font-bold text-slate-600 shadow-sm">
            ...
            {#if nextClasses.length > 0}
                <div class="space-y-2.5">
                    {#each nextClasses.slice(0, 3) as sch, i (i)}
                        <div class="flex items-center gap-4 p-3 rounded-xl bg-white border border-slate-100 shadow-sm transition-transform hover:scale-[1.02]">
                            ...
                            <div class="w-1 h-8 rounded-full shrink-0" style="background-color: {sch.color}"></div>
                    ...
                    {#if nextClasses.length > 3}
                        <div class="text-center pt-2">
                            <span class="text-xs font-bold text-slate-400">+{nextClasses.length - 3} clases más hoy</span>
                        </div>
                    {/if}
                </div>
