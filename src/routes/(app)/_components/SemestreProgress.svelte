<script lang="ts">
    ...
    function getStatusIcon(estado: 'possible' | 'impossible' | 'guaranteed') {
        switch (estado) {
            case 'guaranteed': return CircleCheck;
            case 'impossible': return CircleX;
            case 'possible': default: return Circle;
        }
    }

    function getIconColor(estado: 'possible' | 'impossible' | 'guaranteed') {
        switch (estado) {
            case 'guaranteed': return 'text-success-100';
            case 'impossible': return 'text-error-100';
...
<div class="bg-base-100 rounded-2xl p-5 border border-base-400 shadow-sm h-full">
    <div class="flex justify-between items-start mb-4">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-bold text-content/50 uppercase tracking-widest">Progreso Semestral</h3>
        </div>
